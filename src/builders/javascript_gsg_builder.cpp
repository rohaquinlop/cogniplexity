...
string_view JavaScriptGSGBuilder::name_of(TSNode n, const string &src) {
  ...
  // method_definition: name is under field 'name' too, but can be property_identifier
...
static TSNode js_unwrap_parens(TSNode n) {
  while (!ts_node_is_null(n) && string_view{ts_node_type(n)} == "parenthesized_expression") {
...
static JSBoolOp js_get_bool_op(TSNode n, const string &src) {
  ...
  if (ty == "binary_expression") {
    // operator is an anonymous child; grab operator token text by slicing between children
    // Heuristic: extract full text of node, look for '&&' or '||'
...
unsigned int JavaScriptGSGBuilder::js_count_bool_ops_expr(TSNode n, int nesting, const string &src) {
  ...
  if (ty == "binary_expression") {
    ...
    TSNode left = ts_node_child_by_field_name(n, "left", 4);
    TSNode right = ts_node_child_by_field_name(n, "right", 5);
    return base + alts + js_count_bool_ops_expr(left, nesting, src) + js_count_bool_ops_expr(right, nesting, src);
  }
  ...
  if (ty == "conditional_expression") {
    ...
    int m = ts_node_named_child_count(n);
    for (int i = 0; i < m; ++i) c += js_count_bool_ops_expr(ts_node_named_child(n, i), nesting, src);
    return c;
  }
  unsigned int total = 0;
  int m = ts_node_named_child_count(n);
  for (int i = 0; i < m; ++i) total += js_count_bool_ops_expr(ts_node_named_child(n, i), nesting, src);
  return total;
}
      ...
      if (!ts_node_is_null(body)) {
        ...
        for (int k = 0; k < kmax; ++k) {
          ...
          if (cty == "switch_case" || cty == "switch_default") {
            ...
            if (!ts_node_is_null(cbody)) build_block_children(cbody, src, cs.children);
...
GSGNode JavaScriptGSGBuilder::build_if(TSNode n, const string &src) {
  ...
  if (!ts_node_is_null(cond)) g.addl_cost += js_count_bool_ops_expr(cond, /*nesting=*/0, src);
...
GSGNode JavaScriptGSGBuilder::build_do_while(TSNode n, const string &src) {
  ...
  TSNode cond = ts_node_child_by_field_name(n, "condition", 9);
  if (!ts_node_is_null(cond)) g.addl_cost += js_count_bool_alternations(cond, src);
  TSNode body = ts_node_child_by_field_name(n, "body", 4);
  if (!ts_node_is_null(body)) build_block_children(body, src, g.children);
  return g;
}
