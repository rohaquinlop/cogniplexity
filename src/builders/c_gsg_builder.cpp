...
static string compute_ancestor_qual(TSNode n, const string &src) {
  ...
  while (!ts_node_is_null(cur)) {
    ...
    if (ty == "class_specifier" || ty == "struct_specifier" || ty == "union_specifier") {
      ...
      if (!ts_node_is_null(nm)) parts.push_back(string(string_view(src).substr(ts_node_start_byte(nm), ts_node_end_byte(nm) - ts_node_start_byte(nm))));
...
std::vector<GSGNode> CLikeGSGBuilder::build_functions(TSNode root, const string &src) {
  std::vector<GSGNode> funcs;
  collect_functions_in_scope(root, src, /*qual*/"", funcs);
  return funcs;
}

void CLikeGSGBuilder::collect_functions_in_scope(TSNode n, const string &src, const string &qual, std::vector<GSGNode> &out) {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    if (ty == "function_definition") {
      ...
      string merged;
      if (qual.empty()) merged = aq;
      else if (aq.empty()) merged = qual;
      else if (aq == qual) merged = qual;
      else if (aq.rfind(qual + "::", 0) == 0) merged = aq;
      else if (qual.rfind(aq + "::", 0) == 0) merged = qual;
      else merged = qual + "::" + aq;
      ...
      for (int ti = 0; ti < tn; ++ti) {
        ...
        } else if (ity == "declaration" || ity == "class_specifier" || ity == "struct_specifier" || ity == "namespace_definition" || ity == "template_declaration") {
...
string CLikeGSGBuilder::function_name_from_declarator(TSNode decl, const string &src) {
  ...
  if (p != string::npos) {
    ...
    auto trim = [](string &s) {
      ...
      if (a == string::npos) s.clear(); else s = s.substr(a, b - a + 1);
    };
    trim(pre);
    // remove leading pointer/reference/parentheses wrappers
    while (!pre.empty() && (pre[0] == '*' || pre[0] == '&' || pre[0] == '(')) pre.erase(pre.begin());
    trim(pre);
  ...
  for (int i = 0; i < n; ++i) {
    ...
    string ty = t(ch);
    if (ty == "identifier" || ty == "field_identifier") return string(slice(src, ch));
...
GSGNode CLikeGSGBuilder::build_function(TSNode n, const string &src) {
  GSGNode g; g.kind = GSGNodeKind::Function; g.loc = loc(n);
...
GSGNode CLikeGSGBuilder::build_function(TSNode n, const string &src, const string &qual) {
...
void CLikeGSGBuilder::build_block_children(TSNode n, const string &src, std::vector<GSGNode> &out) {
  int m = ts_node_named_child_count(n);
  for (int i = 0; i < m; ++i) {
    ...
    if (ty == "if_statement") out.emplace_back(build_if(s, src));
    else if (ty == "while_statement") out.emplace_back(build_while(s, src));
    else if (ty == "for_statement") out.emplace_back(build_for(s, src));
    else if (ty == "do_statement") out.emplace_back(build_do_while(s, src));
    else if (ty == "switch_statement") {
      GSGNode sw; sw.kind = GSGNodeKind::Switch; sw.loc = loc(s);
      int cn = ts_node_named_child_count(s);
      for (int j = 0; j < cn; ++j) {
        TSNode cc = ts_node_named_child(s, j);
        string cty = t(cc);
        if (cty == "case_statement" || cty == "default_statement") {
          GSGNode cs; cs.kind = GSGNodeKind::Case; cs.loc = loc(cc);
          int bn = ts_node_named_child_count(cc);
          for (int k = 0; k < bn; ++k) {
            TSNode bch = ts_node_named_child(cc, k);
            // heuristically treat nested statements under case
            if (ts_node_is_named(bch)) build_block_children(bch, src, cs.children);
          }
      ...
      if (!ts_node_is_null(arg)) {
        ...
        if (cost) { GSGNode e; e.kind = GSGNodeKind::Expr; e.loc = loc(s); e.addl_cost = cost; out.emplace_back(std::move(e)); }
      }
      ...
      if (ts_node_named_child_count(s) > 0) {
        ...
        if (cost) { GSGNode e; e.kind = GSGNodeKind::Expr; e.loc = loc(expr); e.addl_cost = cost; out.emplace_back(std::move(e)); }
      }
    } else if (ty == "declaration") {
      unsigned int sum = 0;
      int dn = ts_node_named_child_count(s);
      for (int di = 0; di < dn; ++di) sum += c_count_bool_ops_expr(ts_node_named_child(s, di), /*nesting=*/0, src);
      if (sum) { GSGNode e; e.kind = GSGNodeKind::Expr; e.loc = loc(s); e.addl_cost = sum; out.emplace_back(std::move(e)); }
    }
  }
}

GSGNode CLikeGSGBuilder::build_if(TSNode n, const string &src) {
  GSGNode g; g.kind = GSGNodeKind::If; g.loc = loc(n);
  TSNode cond = ts_node_child_by_field_name(n, "condition", 9);
  if (!ts_node_is_null(cond)) g.addl_cost += c_count_bool_ops_expr(cond, /*nesting=*/0, src);
  ...
  if (!ts_node_is_null(alt)) {
    ...
    if (ty == "if_statement") {
      ...
      GSGNode el; el.kind = GSGNodeKind::Else; el.loc = loc(alt);
...
GSGNode CLikeGSGBuilder::build_while(TSNode n, const string &src) {
  GSGNode g; g.kind = GSGNodeKind::While; g.loc = loc(n);
  TSNode cond = ts_node_child_by_field_name(n, "condition", 9);
  if (!ts_node_is_null(cond)) g.addl_cost += c_count_bool_ops_expr(cond, /*nesting=*/0, src);
  TSNode body = ts_node_child_by_field_name(n, "body", 4);
  if (!ts_node_is_null(body)) build_block_children(body, src, g.children);
...
GSGNode CLikeGSGBuilder::build_for(TSNode n, const string &src) {
  GSGNode g; g.kind = GSGNodeKind::For; g.loc = loc(n);
  TSNode cond = ts_node_child_by_field_name(n, "condition", 9);
  if (!ts_node_is_null(cond)) g.addl_cost += c_count_bool_ops_expr(cond, /*nesting=*/0, src);
  TSNode body = ts_node_child_by_field_name(n, "body", 4);
  if (!ts_node_is_null(body)) build_block_children(body, src, g.children);
...
GSGNode CLikeGSGBuilder::build_do_while(TSNode n, const string &src) {
  GSGNode g; g.kind = GSGNodeKind::DoWhile; g.loc = loc(n);
  TSNode cond = ts_node_child_by_field_name(n, "condition", 9);
  if (!ts_node_is_null(cond)) g.addl_cost += c_count_bool_ops_expr(cond, /*nesting=*/0, src);
  TSNode body = ts_node_child_by_field_name(n, "body", 4);
  if (!ts_node_is_null(body)) build_block_children(body, src, g.children);
...
static unsigned int c_count_bool_alternations(TSNode n, const string &src) {
  ...
  if (ty == "binary_expression") {
    ...
    int curr = (s.find("&&") != string::npos) ? 0 : ((s.find("||") != string::npos) ? 1 : -1);
    auto sl = sv_slice(src, left);
    auto sr = sv_slice(src, right);
    int lb = (sl.find("&&") != string::npos) ? 0 : ((sl.find("||") != string::npos) ? 1 : -1);
    int rb = (sr.find("&&") != string::npos) ? 0 : ((sr.find("||") != string::npos) ? 1 : -1);
    if (lb != -1 && curr != lb) c++;
    if (rb != -1 && curr != rb) c++;
...
unsigned int CLikeGSGBuilder::c_count_bool_ops_expr(TSNode n, int nesting, const string &src) {
  ...
  if (ty == "binary_expression") {
    ...
    TSNode left = ts_node_child_by_field_name(n, "left", 4);
    TSNode right = ts_node_child_by_field_name(n, "right", 5);
    return base + alts + c_count_bool_ops_expr(left, nesting, src) + c_count_bool_ops_expr(right, nesting, src);
  }
  ...
  if (ty == "conditional_expression") {
    ...
    int m = ts_node_named_child_count(n);
    for (int i = 0; i < m; ++i) c += c_count_bool_ops_expr(ts_node_named_child(n, i), nesting, src);
    return c;
  }
  unsigned int total = 0;
  int m = ts_node_named_child_count(n);
  for (int i = 0; i < m; ++i) total += c_count_bool_ops_expr(ts_node_named_child(n, i), nesting, src);
  return total;
}

void CLikeGSGBuilder::collect_lambdas_in_node(TSNode n, const string &src, std::vector<GSGNode> &out) {
  ...
  int m = ts_node_named_child_count(n);
  for (int i = 0; i < m; ++i) collect_lambdas_in_node(ts_node_named_child(n, i), src, out);
}

GSGNode CLikeGSGBuilder::build_lambda(TSNode n, const string &src) {
  GSGNode g; g.kind = GSGNodeKind::Function; g.loc = loc(n);
  // Name as lambda@row:col
  g.name = "lambda@" + std::to_string(g.loc.row) + ":" + std::to_string(g.loc.start_col);
  TSNode body = ts_node_child_by_field_name(n, "body", 4);
